...
impl NoiseHubSessions {
    pub fn new(local_private_key: [u8; 32]) -> Self {
        Self { local_private_key, sessions: HashMap::new() }
    }
    ...
    pub fn handshake_frame(&mut self, session_id: &str, frame: &[u8]) -> Result<Option<Vec<u8>>> {
        if frame.len() > MAX_FRAME_BYTES { bail!("Noise frame is too large"); }
        if !self.sessions.contains_key(session_id) {
            let params: NoiseParams = NOISE_PARAMS.parse()?;
            let responder = Builder::new(params).local_private_key(&self.local_private_key).build_responder()?;
            self.sessions.insert(session_id.to_owned(), SessionState::Handshake(responder));
        }
        let state = self.sessions.get_mut(session_id).expect("inserted above");
        let SessionState::Handshake(handshake) = state else { bail!("Noise handshake is already complete"); };
        ...
        if handshake.is_handshake_finished() {
            ...
            let SessionState::Handshake(handshake) = state else { unreachable!() };
            self.sessions.insert(session_id.to_owned(), SessionState::Transport(handshake.into_transport_mode()?));
    ...
    pub fn receive(&mut self, session_id: &str, ciphertext: &[u8]) -> Result<Vec<u8>> {
        if ciphertext.len() > MAX_FRAME_BYTES { bail!("Noise frame is too large"); }
        let Some(SessionState::Transport(transport)) = self.sessions.get_mut(session_id) else { bail!("Noise session is not established"); };
    ...
    pub fn send(&mut self, session_id: &str, plaintext: &[u8]) -> Result<Vec<u8>> {
        if plaintext.len() > MAX_FRAME_BYTES { bail!("Noise message is too large"); }
        let Some(SessionState::Transport(transport)) = self.sessions.get_mut(session_id) else { bail!("Noise session is not established"); };
...
mod tests {
    ...
    fn xx_handshake_encrypts_bidirectional_messages() {
        ...
        let mut initiator = builder.local_private_key(&keypair.private).build_initiator().unwrap();
        ...
        let response = hub.handshake_frame("session-1", &frame[..size]).unwrap().unwrap();
        ...
        assert_eq!(hub.receive("session-1", &frame[..size]).unwrap(), b"wallet_hello");
